				 ...
				 Buffer recent_buffer)
{
	...
	{
		bufHdr = GetBufferDescriptor(recent_buffer - 1);//to do
 ...
 */
int getMaxTimeStamp1(){//returns the maximum timestamp of all the buffers
	int ans =0;
	uint32 local_temp_state=0;//used in accuring the lock on buffer
	BufferDesc *temp;//used to iterate the buffer pool
	for(int i=0;i<NBuffers;i++){
		temp = GetBufferDescriptor(i);//returns buffer descriptor with id i
			local_temp_state = LockBufHdr(temp);//locking temp
			if(ans<temp->timestamp){//checking if maximum timestamp
				ans = temp->timestamp;//updating ans
			}
			UnlockBufHdr(temp,local_temp_state);//unlocking the buffer
	}
				  ...
				  BufferAccessStrategy strategy, bool *hit)
{
	...
	*hit = false;
	...
	{
		elog(LOG, "Get buf %d\n", bufHdr->buf_id);
		bufHdr->timestamp=getMaxTimeStamp1()+1;//if the buffer is found in the buffer pool we return the buf and update its timestamp to maximum timestamp+1
		if (!isExtend)
		{
